...
| Player context is narrow | Arrows steer the player only when fullscreen or focus is inside `.video-js`. `←/→` seek, `↑` reveals controls, and `↓` calls `moveFocusOutside` to reach the page content below (otherwise the D-pad is trapped in the player). In fullscreen `↑/↓` both reveal controls since there is nowhere to go. |
...
| OK enters fullscreen from the watch page | `onEnter` lets the browser activate a focused player control (big play button, control bar), then enters fullscreen (`video.js` `requestFullscreen`, else the Fullscreen API) and plays; in fullscreen it toggles play/pause. Do not go back to swallowing OK. |
